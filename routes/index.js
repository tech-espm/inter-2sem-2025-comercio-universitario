...
router.get("/produtos", wrap(async (req, res) => {
	...
	await sql.connect(async sql => {
		...
		produtos = await sql.query("select id, nome, descricao, valor, nota, date_format(data, '%d/%m/%Y') data from produto");
...
router.post("/api/cadastrar", upload.single("imagem"), wrap(async (req, res) => {
	...
	await sql.connect(async sql => {
		...
		await sql.query("insert into produto (nome, descricao, valor, nota, data) values (?, ?, ?, ?, current_timestamp())", parametros);
